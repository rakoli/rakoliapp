<script>
    ...
    function updateTownList(regionCode, townlistElementId){
        ...
        @if(auth()->user()->type == \App\Utils\Enums\UserTypeEnum::ADMIN->value)
            var url = "{{route('admin.business.branches.townlistAjax')}}";
        @else
            var url = "{{route('business.branches.townlistAjax')}}";
        @endif
    ...
    function updateAreaList(townCode, arealistElementId){
        ...
        // Configure the GET request
        @if(auth()->user()->type == \App\Utils\Enums\UserTypeEnum::ADMIN->value)
            var url = "{{route('admin.business.branches.arealistAjax')}}";
        @else
            var url = "{{route('business.branches.arealistAjax')}}";
        @endif
